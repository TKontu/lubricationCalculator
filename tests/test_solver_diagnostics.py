...
from lubrication_flow_package.config.unified_config import UnifiedConfig
...
class SolverDiagnostics:
    ...
    def __init__(self, config_path: str):
        ...
        self.config = UnifiedConfig.from_file(self.config_path)
        self.network = FlowNetwork.from_unified_config(self.config)
    ...
    def analyze_linear_initial_guess(self):
        ...
        linear_solver = NodalMatrixSolver(self.config.simulation)
    ...
    def analyze_jacobian_properties(self, solver_type='tree'):
        ...
        if solver_type == 'tree':
            solver = TreeSolver(self.config.simulation)
        else:
            solver = RobustNonLinearSolver(self.config.simulation)
        ...
        if solver_type == 'tree':
            ...
            def patched_solve(network):
                ...
                for i in range(max_iterations):
                    ...
                    # Check convergence
                    if residual_norm < self.config.simulation.tolerance:
                        break
...
def test_complex_network_diagnostics():
    ...
    config_path = "/mnt/c/code/lubrication3/examples/complex_network_modified.json"
